...
export class PalletsController {
  ...
  async upload(
    ...
    if (!file) {
      session.palletReport = {
        total: 0,
        created: 0,
        skippedDuplicates: 0,
        rejected: 0,
        errors: [],
        error: 'Файл не загружен. Выберите .xlsx файл манифеста.',
      };
    ...
    if (!file.originalname.toLowerCase().endsWith('.xlsx')) {
      session.palletReport = {
        ...
        errors: [],
        error:
          'Неподдерживаемый тип файла. Принимаются только файлы .xlsx.',
      };
      return res.redirect('/warehouse/pallets');
    }

    try {
      const report = await this.palletImport.importFromBuffer(
        file.buffer,
        session.user?.id,
      );

      // Если есть ошибки — генерируем CSV
      if (report.errors.length > 0) {
        const csv = this.palletImport.generateCsvReport(report.errors);
        const csvBase64 = Buffer.from(csv, 'utf-8').toString('base64');
        session.palletCsv = csvBase64;
      }
      ...
      session.palletReport = {
        ...
        error:
          err?.message ||
          'Не удалось обработать файл. Проверьте формат манифеста.',
      };
    }
  ...
  async downloadCsv(
    ...
    res.setHeader(
      'Content-Disposition',
      'attachment; filename="pallet-import-errors.csv"',
    );
    res.send(csv);
  }
}
